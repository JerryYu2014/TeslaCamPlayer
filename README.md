...
- **片段导出与合并**  
  将选定时间段导出为独立视频文件，支持基础合并处理（依赖 ffmpeg）。
- **现代化桌面 UI**  
  使用 PyQt5 + qt-material + QtAwesome，提供类 Material 风格的桌面界面。
- **系统通知与提醒**  
  在 Windows 上使用 `win11toast` 提供系统级通知。
...
├─ TeslaCamPlayer.pkgproj        # Packages 工程（macOS）
