...
- 支持配置系统通知、邮件通知、企业微信机器人通知等，在关键操作（如合成导出完成）时提醒你。
...
- 你可以选择直接从 GitHub 下载并启动安装程序：下载过程在独立的进度窗口中进行，不会卡死主界面，窗口中会展示实时进度条、下载链接（支持一键复制）和本地保存路径（支持一键打开所在文件夹），并且可以为更新下载单独设置 HTTP 代理。
